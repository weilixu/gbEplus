...
public class ReverseTranslator {
	...
	private String energyPlusVersion;// specify the version of EnergyPlus that
										// one wish to convert
	...
	private void translateGBXML(Element gbXML, Document doc) {
		...
		if (temperatureUnit.equalsIgnoreCase("F")) {
			...
			// TODO should give a warning: "No temperature unit specified, using
			// C"
			m_temperatureUnit = "C";
		}
		...
		// {SquareKilometers, SquareMeters, SquareCentimeters,
		// SquareMillimeters, SquareMiles, SquareYards, SquareFeet,
		// SquareInches}
		areaUnit = gbXML.getAttributeValue("areaUnit");

		// {CubicKilometers, CubicMeters, CubicCentimeters, CubicMillimeters,
		...
		if (materialElements.isEmpty()) {
			...
			for (int i = 0; i < dataPluginList.size(); i++) {
				...
				if (dataAPI.dataBaseType().equals(DataBaseType.CONSTRUCTION)) {
					if(checkCoRequiredAPIs(dataAPI.requiredCoPlugins(), dataPluginList)){
						envelopeTranslator.generateAssumptions(file, dataAPI);
					}
					break;// only allow one construction plugin to be activated
				}
			}
		} else {
		...
		if (materialElements.isEmpty()) {
			...
			for (int i = 0; i < dataPluginList.size(); i++) {
				EnergyPlusDataAPI dataAPI = dataPluginList.get(i);
				if (dataAPI.dataBaseType().equals(DataBaseType.THERMALZONE)) {
					if(checkCoRequiredAPIs(dataAPI.requiredCoPlugins(), dataPluginList)){
						dataAPI.writeInObjects(this);
					}
					//envelopeTranslator.generateAssumptions(file, dataAPI);
					break;// only allow one construction plugin to be activated
				}
		...
		// TODO if no HVAC system, then baseline system selection should be
		// implemented.
	...
	private boolean checkCoRequiredAPIs(HashMap<DataBaseType, String> requiredPluginMap, List<EnergyPlusDataAPI> apiList){
		if(requiredPluginMap==null || requiredPluginMap.isEmpty()){
			return true;
		}
		
		Iterator<DataBaseType> typeList = requiredPluginMap.keySet().iterator();
		while(typeList.hasNext()){
			DataBaseType type = typeList.next();
			String plugInName = requiredPluginMap.get(type);
			if(!checkCoRequiredAPI(type, plugInName, apiList)){
				return false;
			}
	...
	private boolean checkCoRequiredAPI(DataBaseType type, String plugInName, List<EnergyPlusDataAPI> apiList){
		for(EnergyPlusDataAPI api: apiList){
			if(api.dataBaseType().equals(type)&& api.dataBaseName().equals(plugInName)){
				return true;
			}
		}
		return false;
	...
	public void insertThermalZones(GbXMLThermalZone thermalZone){
	...
	public void exportFile(String idfFilePath) {
		try {
			...
			// GeometryFromIDFFileObject idfConverter = new
			// GeometryFromIDFFileObject();
